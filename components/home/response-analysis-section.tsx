...
export function ResponseAnalysisSection({ t, hasSubmitted }: ResponseAnalysisSectionProps) {
  return (
    <section className="rounded-[2rem] border border-slate-200/70 bg-white/80 p-5 shadow-[0_18px_60px_rgba(148,113,73,0.12)] backdrop-blur sm:p-6 h-full flex flex-col">
      ...
      <div className="mt-5 flex-1 rounded-[1.5rem] bg-gradient-to-br from-emerald-50 to-teal-100 p-5 text-slate-950 flex flex-col gap-4 overflow-y-auto">
        {hasSubmitted ? (
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-emerald-800/60 p-6 text-center">
            <p className="text-sm leading-6">Speak or type a response in the practice section, then press Submit to see your AI response analysis here!</p>
          </div>
        )}
      </div>
    </section>
